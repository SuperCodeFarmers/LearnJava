 ...
 * 	生产饮料、就、调料的厂商买回这个瓶子后，在装东西时，就给这个瓶子加上商标（标签）。
 * 
 ...
 * 	在设计时不确定元素类型，但是在使用时，程序员是知道类型的。
 * 	现在需要一个方式，渠道，让使用者在使用这个集合时，告知这个集合，我里面装的是什么对象。这就需要泛型
 * 
 *  现在在设计这个集合时，元素的类型不确定，所以我爸这个类型用形参表示，让使用者用实参给我确定这个类型
...
public class TestGeneric {
	...
	public void test01() {
		...
		// 使用时，不知道我这里装的是String类型,统统按照Object
		for (Object object : list) {
			// 每次使用元素是，可能需要强转，很麻烦，好比每次打开瓶子，你需要问一下
